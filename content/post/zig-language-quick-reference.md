...
函数经常返回错误联合。这是一个使用 catch 的例子，其中 |err|语法接收错误的值。这称为有效负载捕获，并且在许多地方都有类似的使用。我们将在本章后面更详细地讨论它。

> 旁注：某些语言对 lambda 使用类似的语法 - Zig 的情况并非如此。
...
Zig 的 switch 既可以用作语句，也可以用作表达式。所有分支的类型必须强制为正在switch的类型。所有可能的值必须有一个关联的分支 - 值不能被遗漏。case 不能转至其他分支机构。

switch 语句的示例。需要 else 来匹配没有匹配的情况。
...
Zig 提供一定程度的安全性，在执行过程中可能会发现问题。安全功能可以保持开启或关闭。 Zig 有很多所谓的可检测非法行为的案例，这意味着在安全打开时非法行为会被捕获（引起panic），但在安全关闭时会导致未定义的行为。强烈建议用户在安全的情况下开发和测试他们的软件，尽管这会降低速度。
...
结构体是 Zig 最常见的复合数据类型，允许您定义可以存储一组固定命名字段的类型。 Zig 不保证结构体中字段在内存中的顺序或其大小。与数组一样，结构体也可以使用 T{} 语法整齐地构造。这是声明和填充结构的示例。
